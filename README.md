...
In [Homework Assignment 8](https://github.com/PGE383-HPC-Students/assignment8) we wrote a `Toughness` class that computes a materials toughness given stress/strain data in file.  In this assignment, we will make the integration (i.e. toughness computation) a parallel task.  You should complete the new `ParallelToughness` class, specifically the function `compute_toughness()` shown in [assignment15.py](assignment14.py).  
